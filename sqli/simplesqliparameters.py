...
def verify_password():
    try:
        ...
        conn=sqlite3.connect('users.db')
        q="select  username, password from users where username=? and password=?;"
        cur = conn.cursor()
...
def handle(user):
    ...
    try:
        conn=sqlite3.connect('users.db')
        q="select distinct username, password from users where username=?;"
        cur = conn.cursor()
        cur.execute(q, (user,))
        rows = cur.fetchall();
        conn.commit()
